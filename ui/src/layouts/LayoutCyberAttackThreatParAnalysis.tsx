...
import { useGetTacticTechniqueObservableCountsQuery, useGetTacticTechniqueParCountsQuery, useGetTop5ImpactCountsQuery } from '../../app/services/tacticTechniquesStatsApi';
import { useTheme } from '../contexts/useTheme';

type Props = object;
const LayoutCyberAttackThreatAnalysis: React.FC<Props> = () => {
    ...
    return (
        <>
            <PageBanner title='Cybersecurity Threat Analysis: Precursor Attack Reports' isLoading={false} baseRoute='/quantatative-attack-analysis' />
            <div className="p-10">
                <div className="grid sm:grid-cols-1 gap-4">
                    ...
                    <GenericLoadingErrorWrapper
                        ...
                        renderComponent={(content) => {
                            return (
                                <CardStatusNested title={'Tactic/Technique Occurrence Frequency'} type="normal">
                                    <div className="grid sm:grid-cols-1 xl:grid-cols-1 gap-4">
                                        <CardContent>
                        ...
                        skeletonTypeProps={{
                            ...
                            template: (
                                <div className={`card p-4 overflow-hidden`}>
                                    ...
                                    <div className="grid sm:grid-cols-1 xl:grid-cols-1 gap-4">
                                        {[...Array(1)].map((_, index) => (
                                            <Skeleton key={`skeleton-allAttacksLoadingData-${index}`} height={200} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
                                        ))}
                                    </div>
                                </div>
                            )
                    ...
                    <GenericLoadingErrorWrapper
                        ...
                        data={allAttacksData}
                        error={errorAllAttacksData}
                        keyIndex={'allAttacksDataKeyIndex2'}
                        renderComponent={(content) => {
                            return (
...
export default LayoutCyberAttackThreatAnalysis;
